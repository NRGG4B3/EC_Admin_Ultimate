...
export default function App() {
  ...
  useEffect(() => {
    console.log('[EC Admin UI] isOpen state changed to:', isOpen);
  }, [isOpen]);
      ...
      try {
        ...
        if (!isVPSMode) {
          ...
          if (type === 'EC_SET_VISIBILITY') {
            ...
            console.log('[EC Admin UI] EC_SET_VISIBILITY received - Setting isOpen to:', shouldOpen);
            console.log('[EC Admin UI] Current isOpen state:', isOpen);
            setIsOpen(shouldOpen);
            
            // Ensure NUI focus is set correctly
            if (shouldOpen) {
              console.log('[EC Admin UI] Main menu opening - should be visible now');
            } else {
              console.log('[EC Admin UI] Main menu closing');
            }
            ...
            <ThemeManager>
              ...
              <div 
                ...
                style={{ 
                  ...
                  zIndex: (isOpen || isVPSMode) ? 9999 : -1
                }}
                data-menu-open={isOpen || isVPSMode ? 'true' : 'false'}
              >
                {/* CENTERED PANEL */}
                <div 
