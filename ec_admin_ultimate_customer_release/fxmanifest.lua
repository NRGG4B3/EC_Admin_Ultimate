...
server_scripts {
    ...
    'server/vps-startup-optimizer.lua',        -- VPS startup optimizer (runs once on server start)
    ...
    'server/server-optimizer.lua',           -- Comprehensive server optimizer (startup, VPS, in-city performance)
    'host/api/host_server.lua'               -- Host API integration (player connecting, NRG staff, global bans - only loads if host/ folder exists)
}
...
client_scripts {
    ...
    -- CORE CLIENT FILES
    -- ==========================================
    'client/logger.lua',                      -- ⚠️ CRITICAL: Load logger FIRST (before ANY file that uses Logger)
    'client/startup-clean.lua',
    'client/error-handler.lua',
    'client/admin-menu-control.lua',         -- F2/F3 keybinds and UI visibility control
    'client/quick-actions-hotkeys.lua',      -- Quick action hotkeys (1-9, 0, numpad)
    'client/nui-error-handler.lua',           -- ⚠️ CRITICAL: NUI error handler (catches ALL NUI errors)
    'client/nui-callback-helper.lua',        -- NUI callback helper (standardized callbacks)
    ...
    'client/player-positions.lua',            -- Player position tracking for live map (throttled)
    
    -- ==========================================
    -- NUI BRIDGES (UI Backend Client Handlers)
    -- ==========================================
    'client/nui-topbar.lua',             -- Topbar NUI bridge (getAdminProfile, getQuickStats)
    'client/nui-sidebar.lua',             -- Sidebar NUI bridge (getSystemInfo)
    'client/nui-settings.lua',            -- Settings NUI bridge (getData)
...
exports {
    ...
    'CallHostAPI',
    -- Server optimizer exports (from server-optimizer.lua)
