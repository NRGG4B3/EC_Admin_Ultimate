...
AddEventHandler('onResourceStart', function(resourceName)
    if resourceName == GetCurrentResourceName() then
        ...
        -- Send webhook for server start
        if exports['ec_admin_ultimate'] and exports['ec_admin_ultimate'].TriggerWebhook then
            exports['ec_admin_ultimate']:TriggerWebhook('server_started', {
                title = '🟢 Server Started',
                description = string.format('**%s** has been started', GetConvar('sv_hostname', 'FiveM Server')),
                color = 3066993, -- Green
                fields = {
                    { name = 'Server Name', value = GetConvar('sv_hostname', 'FiveM Server'), inline = true },
                    { name = 'Max Players', value = tostring(GetConvarInt('sv_maxclients', 32)), inline = true },
...
AddEventHandler('onResourceStop', function(resourceName)
    if resourceName == GetCurrentResourceName() then
        ...
        -- Send webhook for server stop
        if exports['ec_admin_ultimate'] and exports['ec_admin_ultimate'].TriggerWebhook then
            exports['ec_admin_ultimate']:TriggerWebhook('server_stopped', {
                title = '🔴 Server Stopped',
                description = string.format('**%s** has been stopped', GetConvar('sv_hostname', 'FiveM Server')),
                color = 15158332, -- Red
                fields = {
                    { name = 'Server Name', value = GetConvar('sv_hostname', 'FiveM Server'), inline = true },
                    { name = 'Time', value = os.date('%Y-%m-%d %H:%M:%S'), inline = false }
...
-- Server restart detection (via onResourceStart after stop)
...
AddEventHandler('onResourceStart', function(resourceName)
    if resourceName == GetCurrentResourceName() and serverRestarting then
        serverRestarting = false
        Wait(2000) -- Wait for server to stabilize
        
        -- Send webhook for server restart
        if exports['ec_admin_ultimate'] and exports['ec_admin_ultimate'].TriggerWebhook then
            exports['ec_admin_ultimate']:TriggerWebhook('server_restarted', {
                title = '🟡 Server Restarted',
                description = string.format('**%s** has been restarted', GetConvar('sv_hostname', 'FiveM Server')),
