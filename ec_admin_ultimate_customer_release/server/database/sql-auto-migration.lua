...
local SQL_EXECUTION_ORDER = {
    'bans.sql',                -- Framework bans table (MUST BE FIRST - used by qbx_core)
...
local function executeSQLStatements(sqlContent, fileName)
    ...
    for line in sqlContent:gmatch('[^\r\n]+') do
        ...
        elseif line:match('%*/') then
            ...
            -- Skip to next iteration (replaced goto continue)
        elseif inMultiLineComment then
            -- Skip to next iteration (replaced goto continue) - do nothing
        elseif not line:match('^%s*%-%-') then
            -- Skip single-line comments (replaced goto continue)
            -- Strip inline comments (everything after -- that's not in quotes)
            -- Simple approach: remove -- and everything after it, but preserve quoted strings
            local cleanedLine = line
            local inString = false
            local quoteChar = nil
            local result = ''
            
            for i = 1, #cleanedLine do
                local char = cleanedLine:sub(i, i)
                local nextChar = cleanedLine:sub(i + 1, i + 1)
                ...
                if not inString then
                    ...
                    result = result .. char
                    if char == quoteChar and cleanedLine:sub(i - 1, i - 1) ~= '\\' then
                        inString = false
                        quoteChar = nil
                    end
                end
