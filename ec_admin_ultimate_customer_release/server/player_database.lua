...
-- Cached online players data (updated periodically, not on every call)
local cachedOnlinePlayers = {}
local cachedOnlinePlayersTime = 0
local ONLINE_PLAYERS_CACHE_TTL = 5 -- Cache for 5 seconds

-- Helper: Get online players data (OPTIMIZED with caching)
local function getOnlinePlayersData()
    -- Check cache first
    local currentTime = getCurrentTimestamp()
    if cachedOnlinePlayersTime > 0 and (currentTime - cachedOnlinePlayersTime) < ONLINE_PLAYERS_CACHE_TTL then
    ...
    -- Process players (lightweight operations only)
    for _, playerId in ipairs(playerList) do
        ...
        if source then
            ...
            -- Get lightweight framework info (cached if possible)
            local frameworkInfo = getPlayerFrameworkInfo(source)
            
            -- Use cached playtime/warnings if available, otherwise fetch (async)
            local playtime = 0
            local warnings = 0
            if primaryId then
                -- Try to get from cache first
                local cacheKey = primaryId .. ':stats'
                if playerCache[cacheKey] and (currentTime - playerCache[cacheKey].timestamp) < CACHE_TTL then
                    playtime = playerCache[cacheKey].playtime or 0
                    warnings = playerCache[cacheKey].warnings or 0
                else
                    -- Fetch async (don't block)
                    CreateThread(function()
                        playtime = getPlayerPlaytime(primaryId) or 0
                        warnings = getPlayerWarningsCount(primaryId) or 0
                        -- Cache result
            ...
            table.insert(players, {
                ...
                ping = GetPlayerPing(source), -- Lightweight call
                playtime = playtime,
                status = 'playing',
                admin = isPlayerAdmin(source), -- Lightweight check
                location = getPlayerLocation(source), -- Lightweight
                online = true,
    ...
    cachedOnlinePlayersTime = currentTime
...
lib.callback.register('ec_admin:playerDatabase:getPlayers', function(source, includeOffline)
    local response = getPlayersData(includeOffline or false)
    return response
end)

-- Callback throttling cache
local callbackThrottle = {}
local THROTTLE_INTERVAL = 2000 -- 2 seconds minimum between calls per source

-- Callback: Get players (via fetchNui/client bridge) - THROTTLED & CACHED
lib.callback.register('ec_admin:getPlayers', function(source, data)
    ...
    local cacheKey = 'getPlayers:' .. tostring(source) .. ':' .. tostring(includeOffline)
...
-- Note: RegisterNUICallback is CLIENT-side only
-- Use lib.callback.register for server-side callbacks
lib.callback.register('ec_admin:playerDatabase:getBans', function(source, activeOnly)
    local response = getBansData(activeOnly or false)
    return response
end)
