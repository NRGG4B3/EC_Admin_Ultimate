...
RegisterNetEvent('ec_admin:client:healSelf', function()
    ...
    print("^2[EC Admin] ✅ Healed and armored^0")
end)
...
RegisterNetEvent('ec_admin:client:maxArmor', function()
    local ped = PlayerPedId()
    SetPedArmour(ped, 100)
    print("^2[EC Admin] ✅ Max armor applied^0")
end)
...
RegisterNetEvent('ec_admin:reviveSelf', function()
    ...
    print("^2[EC Admin] ✅ Revived yourself^0")
end)
...
RegisterNetEvent('ec_admin:cleanClothes', function()
    ...
    print("^2[EC Admin] ✅ Clothes cleaned^0")
end)
...
RegisterNetEvent('ec_admin:washPlayer', function()
    ...
    ClearPedEnvDirt(ped)
    ResetPedVisibleDamage(ped)
    print("^2[EC Admin] ✅ Player washed^0")
end)
...
RegisterNetEvent('ec_admin:clearBlood', function()
    local ped = PlayerPedId()
    ClearPedBloodDamage(ped)
    print("^2[EC Admin] ✅ Blood cleared^0")
end)
...
RegisterNetEvent('ec_admin:client:teleportToCoords', function(x, y, z)
    if not x or not y or not z then
        print("^1[EC Admin] ❌ Invalid coordinates for teleport^0")
        return
    end
    ...
    if not xNum or not yNum or not zNum then
        print("^1[EC Admin] ❌ Coordinates must be numbers^0")
        return
    end
    ...
    if SafeTeleportToCoords(ped, targetCoords, heading) then
        ...
        print("^1[EC Admin] ❌ Teleport failed (invalid ped or coords)^0")
    end
end)
...
RegisterNetEvent('ec_admin:teleportBack', function()
    if lastTeleportPosition then
        ...
        if SafeTeleportToCoords(ped, lastTeleportPosition, GetEntityHeading(ped)) then
            print("^2[EC Admin] ✅ Teleported back^0")
            lastTeleportPosition = nil
        else
            print("^1[EC Admin] ❌ Teleport back failed^0")
        end
    else
        print("^1[EC Admin] ❌ No previous location^0")
    end
end)
...
RegisterNetEvent('ec_admin:client:killPlayer', function()
    ...
    print("^3[EC Admin] ⚠️  You were killed by an admin^0")
end)
...
RegisterNetEvent('ec_admin:client:freezePlayer', function(freeze)
    ...
    if freeze then
        print("^3[EC Admin] ⚠️  You have been frozen^0")
    else
        print("^2[EC Admin] ✅ You have been unfrozen^0")
    end
end)
...
RegisterNetEvent('ec_admin:client:sitPlayer', function()
    ...
    print("^3[EC Admin] ⚠️  You were forced to sit^0")
end)
...
RegisterNetEvent('ec_admin:client:dragPlayer', function(adminId, targetId)
    ...
    if DoesEntityExist(adminPed) and DoesEntityExist(targetPed) then
        ...
        if isDragging then
            ...
            print("^3[EC Admin] ⚠️  You are being dragged^0")
        else
            DetachEntity(targetPed, true, false)
            print("^2[EC Admin] ✅ You are no longer being dragged^0")
        end
    end
end)
...
RegisterNetEvent('ec_admin:client:cuffPlayer', function(cuff)
    ...
    if cuff then
        ...
        print("^3[EC Admin] ⚠️  You have been cuffed^0")
    else
        ClearPedSecondaryTask(ped)
        SetEnableHandcuffs(ped, false)
        SetPedCanPlayGestureAnims(ped, true)
        FreezeEntityPosition(ped, false)
        print("^2[EC Admin] ✅ You have been uncuffed^0")
    end
end)
...
RegisterNetEvent('ec_admin:client:removeMask', function()
    ...
    print("^3[EC Admin] Your mask was removed^0")
end)
...
RegisterNetEvent('ec_admin:client:removeHat', function()
    local ped = PlayerPedId()
    ClearPedProp(ped, 0)
    print("^3[EC Admin] Your hat was removed^0")
end)
...
RegisterNetEvent('ec_admin:client:spectatePlayer', function(targetId)
    ...
    if spectatingPlayer == targetId then
        ...
        print("^3[EC Admin] Stopped spectating^0")
    else
        -- Start spectating
        ...
        if DoesEntityExist(targetPed) then
            ...
            print("^2[EC Admin] ✅ Spectating player^0")
        end
    end
end)
...
RegisterNetEvent('ec_admin:client:slapPlayer', function()
    ...
    print("^3[EC Admin] ⚠️  You have been slapped!^0")
end)
...
RegisterNetEvent('ec_admin:client:stripWeapons', function()
    ...
    print("^3[EC Admin] Your weapons have been removed^0")
end)
...
RegisterNetEvent('ec_admin:client:changePlate', function(plate)
    ...
    if vehicle ~= 0 then
        ...
        print("^1[EC Admin] ❌ You must be in a vehicle^0")
    end
end)
...
RegisterNetEvent('ec_admin:unlockVehicle', function()
    ...
    if vehicle ~= 0 then
        ...
        print("^2[EC Admin] ✅ Vehicle unlocked^0")
    else
        print("^1[EC Admin] ❌ No vehicle nearby^0")
    end
end)
...
RegisterNetEvent('ec_admin:lockVehicle', function()
    ...
    if vehicle ~= 0 then
        ...
        print("^2[EC Admin] ✅ Vehicle locked^0")
    else
        print("^1[EC Admin] ❌ No vehicle nearby^0")
    end
end)
...
RegisterNetEvent('ec_admin:hopIntoDriver', function()
    ...
    if vehicle ~= 0 then
        ...
        print("^2[EC Admin] ✅ Hopped into driver seat^0")
    else
        print("^1[EC Admin] ❌ No vehicle nearby^0")
    end
end)
...
RegisterNetEvent('ec_admin:client:garageAll', function()
    ...
    print("^2[EC Admin] ✅ Sent all vehicles to garage^0")
end)
