...
-- Get inventory data with all players
lib.callback.register('ec_admin:getInventoryData', function(source, _)
    ...
    for _, playerId in ipairs(allPlayers) do
        ...
        if playerName then
            ...
            table.insert(players, { id = pId, name = playerName, citizenid = GetPlayerIdentifier(pId, 0), itemCount = #inventory.items, weight = inventory.weight, maxWeight = inventory.maxWeight, online = true })
        end
    end
    local allItems = GetAllItems()
    local stats = { totalPlayers = #players, totalItems = 0, totalWeight = 0, uniqueItems = 0 }
    ...
    return { success = true, data = { players = players, items = allItems, stats = stats, inventorySystem = InventorySystem } }
end)
...
lib.callback.register('ec_admin:getPlayerInventory', function(source, data)
    ...
    return { success = true, playerId = targetId, playerName = playerName, inventory = inventory }
end)
...
Logger.Info('Inventory callbacks loaded')