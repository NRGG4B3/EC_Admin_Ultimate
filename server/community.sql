...
CREATE TABLE IF NOT EXISTS `ec_community_groups` (
  ...
  `description` TEXT,
  `group_type` VARCHAR(50) DEFAULT 'custom', -- custom, gang, crew, organization
...
CREATE TABLE IF NOT EXISTS `ec_community_group_members` (
  ...
  `role` VARCHAR(50) DEFAULT 'member', -- leader, admin, member
...
CREATE TABLE IF NOT EXISTS `ec_community_events` (
  ...
  `description` TEXT,
  `event_type` VARCHAR(50) DEFAULT 'custom', -- custom, race, tournament, giveaway, meeting
...
CREATE TABLE IF NOT EXISTS `ec_community_achievements` (
  ...
  `name` VARCHAR(100) NOT NULL,
  `description` TEXT,
  `category` VARCHAR(50) DEFAULT 'general', -- general, combat, social, economic, exploration
  `icon` VARCHAR(50) DEFAULT 'trophy',
  `points` INT DEFAULT 10,
  `requirement_type` VARCHAR(50) DEFAULT 'manual', -- manual, playtime, kills, money, etc.
...
CREATE TABLE IF NOT EXISTS `ec_community_player_achievements` (
  ...
  `unlocked_by` VARCHAR(50), -- Admin who granted it (if manual)
...
CREATE TABLE IF NOT EXISTS `ec_community_actions_log` (
  ...
  `action_type` VARCHAR(50) NOT NULL, -- create_group, create_event, create_achievement, create_announcement, delete_group, etc.
  `target_type` VARCHAR(50), -- group, event, achievement, announcement
  `target_id` INT,
