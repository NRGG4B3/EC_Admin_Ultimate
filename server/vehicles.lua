...
local function getVehicleDisplayName(model)
    ...
    if displayName and displayName ~= 'NULL' then
        return displayName
    end
    -- Fallback: Capitalize model name
...
local function getVehicleClassName(model)
    ...
    return VEHICLE_CLASSES[class] or 'Unknown'
end

-- Helper: Check if vehicle model is valid
local function isVehicleModelValid(model)
    local hash = GetHashKey(model)
    return IsModelInCdimage(hash) or IsModelValid(hash)
end
