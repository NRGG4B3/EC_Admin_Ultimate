...
CREATE TABLE IF NOT EXISTS `ec_whitelist_applications` (
  ...
  `deny_reason` TEXT,
  `application_data` TEXT, -- JSON object for custom fields
  INDEX `idx_identifier` (`identifier`),
  INDEX `idx_status` (`status`),
...
CREATE TABLE IF NOT EXISTS `ec_whitelist_actions_log` (
  ...
  `action_type` VARCHAR(50) NOT NULL, -- add, update, remove, approve, deny, create_role, etc.
  `target_type` VARCHAR(50), -- entry, application, role
  `target_id` INT,
...
CREATE TABLE IF NOT EXISTS `ec_whitelist_join_attempts` (
  ...
  `discord_id` VARCHAR(50),
  `ip_address` VARCHAR(45),
  `result` VARCHAR(20) NOT NULL, -- allowed, denied_whitelist, denied_banned, denied_other
  `reason` TEXT,
