...
CREATE TABLE IF NOT EXISTS `ec_moderation_reports` (
  ...
  `reason` TEXT NOT NULL,
  `category` VARCHAR(50) DEFAULT 'other', -- harassment, cheating, griefing, other
...
CREATE TABLE IF NOT EXISTS `ec_moderation_action_logs` (
  ...
  `admin_id` VARCHAR(50) NOT NULL,
  `admin_name` VARCHAR(100) NOT NULL,
  `action_type` VARCHAR(50) NOT NULL, -- warn, kick, mute, unmute, remove_warning, update_report
  `target_id` VARCHAR(50) NULL,
  `target_name` VARCHAR(100) NULL,
  `reason` TEXT NULL,
  `details` TEXT NULL, -- JSON string for additional details
  `created_at` BIGINT NOT NULL,
  INDEX `idx_admin_id` (`admin_id`),
