...
-- Callback: Get inventory data
RegisterNUICallback('inventory:getData', function(data, cb)
    local response = getInventoryData()
    
    -- Send NUI message to client
    -- Note: RegisterNUICallback doesn't have source, so we can't send directly
    -- The response will be sent via cb() and the client bridge will forward it
...
-- Callback: Remove item from player
